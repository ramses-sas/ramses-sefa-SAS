...
if [ "$IS_REMOTE" = "no" ]; then
  ...
  docker create -p $PORT_MAPPING -e HOST=$HOST -e SERVER_PORT=$SERVICE_PORT -e EUREKA_IP_PORT=$EUREKA_IP_PORT -e API_GATEWAY_IP_PORT=$API_GATEWAY_IP_PORT -e MYSQL_IP_PORT=$MYSQL_IP_PORT -i -t --name $SERVICE_IMPLEMENTATION_NAME $SERVICE_IMPLEMENTATION_NAME
else
  PrintSuccess "4/4 - Creating container $SERVICE_IMPLEMENTATION_NAME in remote mode"
  docker create -p $PORT_MAPPING -e HOST=$HOST -e SERVER_PORT=$SERVICE_PORT -i -t --name $SERVICE_IMPLEMENTATION_NAME $SERVICE_IMPLEMENTATION_NAME
fi
