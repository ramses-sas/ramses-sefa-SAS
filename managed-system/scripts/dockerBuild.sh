...
if [ "$IS_REMOTE" = "no" ]; then
  PrintSuccess "4/4 - Creating container $SERVICE_NAME in local mode"
  docker create -p $PORT_MAPPING -e HOST=$HOST -e EUREKA_IP_PORT=$EUREKA_IP_PORT -e API_GATEWAY_IP_PORT=$API_GATEWAY_IP_PORT -e MYSQL_SERVER=$MYSQL_SERVER -i -t --name $SERVICE_NAME $SERVICE_NAME
else
  PrintSuccess "4/4 - Creating container $SERVICE_NAME in remote mode"
  docker create -p $PORT_MAPPING -e HOST=$HOST -i -t --name $SERVICE_NAME $SERVICE_NAME
fi
