...
public class InstrumentationAspect {
    ...
    @Pointcut("execution(public * it.polimi.dummymanagedsystem.bservice.domain.RandGeneratorService.*(..))")
    public void bServiceMethods() {}

	/* metodi di log */ 
    private void logInvocation(JoinPoint joinPoint) {
        final String args = Arrays.toString(joinPoint.getArgs());
        final String methodName = joinPoint.getSignature().getName().replace("(..)", "()");
        log.info("CALL BService.{} {}", methodName, args);
    }

    private void logTermination(JoinPoint joinPoint, Object retValue) {
        final String args = Arrays.toString(joinPoint.getArgs());
        final String methodName = joinPoint.getSignature().getName().replace("(..)", "()");
        log.info("     BService.{} {} -> {}", methodName, args, retValue.toString());
    }

    private void logException(JoinPoint joinPoint, Object exception) {
        final String args = Arrays.toString(joinPoint.getArgs());
        final String methodName = joinPoint.getSignature().getName().replace("(..)", "()");
        log.info("     ERROR IN BService.{} {} -> {}", methodName, args, exception.toString());
    }

    /* Eseguito prima dell'esecuzione del metodo */
    @Before("bServiceMethods()")
    ...
    @AfterReturning(value="bServiceMethods()", returning="retValue")
    ...
    @AfterThrowing(value="bServiceMethods()", throwing="exception")
