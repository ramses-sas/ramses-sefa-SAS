...
public interface MetricsRepository extends CrudRepository<InstanceMetrics, Long> {
    ...
    @Query("SELECT m FROM InstanceMetrics m WHERE m.instanceId = :instanceId AND m.timestamp >= (SELECT MAX(m1.timestamp) FROM InstanceMetrics m1 WHERE m1.instanceId = :instanceId AND m.status=it.polimi.saefa.knowledge.persistence.domain.architecture.InstanceStatus.SHUTDOWN) ORDER BY m.timestamp DESC")
    ...
    // = it.polimi.saefa.knowledge.persistence.domain.architecture.InstanceStatus.
    @Query("SELECT m FROM InstanceMetrics m WHERE m.instanceId = :instanceId AND m.status=it.polimi.saefa.knowledge.persistence.domain.architecture.InstanceStatus.ACTIVE " +
    ...
    @Query("SELECT m FROM InstanceMetrics m WHERE m.instanceId = :instanceId " +
            "AND (m.status=it.polimi.saefa.knowledge.persistence.domain.architecture.InstanceStatus.FAILED " +
            "AND m.timestamp = (SELECT MAX(m2.timestamp) FROM InstanceMetrics m2 WHERE m2.instanceId = :instanceId AND m2.status=it.polimi.saefa.knowledge.persistence.domain.architecture.InstanceStatus.FAILED) " +
            "OR (m.status=it.polimi.saefa.knowledge.persistence.domain.architecture.InstanceStatus.ACTIVE AND m.timestamp = (SELECT MIN(m3.timestamp) FROM InstanceMetrics m3 WHERE m3.instanceId = :instanceId AND m3.status=it.polimi.saefa.knowledge.persistence.domain.architecture.InstanceStatus.ACTIVE AND m3.timestamp> (SELECT MAX(m4.timestamp) FROM InstanceMetrics m4 WHERE m4.instanceId = :instanceId AND m4.status=it.polimi.saefa.knowledge.persistence.domain.architecture.InstanceStatus.FAILED))))")
