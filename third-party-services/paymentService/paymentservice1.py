...
bind_port = int(env_port) if env_port else 48090
recv_buff = 1024*128
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if cvv == 0:
            ...
            self.send_response(200, "OK")
            self.end_headers()
            self.wfile.write(bytes("Order payed from service 1 after waiting "+str(cvv/10)+" seconds using card "+cardNo, "ascii"))
