...
bind_port = int(env_port) if env_port else 48095
recv_buff = 1024*128
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if number == 0:
            ...
            self.send_response(200, "OK")
            self.end_headers()
            self.wfile.write(bytes("Order delivered to "+address+" from service 1 after waiting "+str(number/10)+" seconds", "ascii"))
